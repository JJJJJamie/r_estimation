...
# N estimation
# solve QP
deaths_noise_10 = deaths + 0.
for i in range(len(deaths)):
    deaths_noise_10[i] = deaths_noise_10[i] + np.random.normal(0, deaths_noise_10[i]*0.1)

M_noise_10, P, q = find_pq_S(deaths_noise_10, p, N_max, dr)
...
N0_noise_10, c = find_n(S0)

S_noise_10 = S0+0.
N_noise_10 = N0_noise_10+0.
c_noise_10 = c+0.
N_range = np.arange(N_start, N_start+len(N))

# cut off last (death_delay_max-death_delay_min-1) estimations
# S_noise_10 = S0[:-(death_delay_max-death_delay_min-1)]
# N_noise_10 = N0_noise_10[:-(death_delay_max-death_delay_min-1)]
# c_noise_10 = c[:-(death_delay_max-death_delay_min-1)]
# N_range = np.arange(N_start, N_start+len(N))

# test results
# plt.plot(S_noise_10)
# plt.plot(nn_cum[N_start:N_start+len(S_noise_10)])
#
# plt.plot(c_noise_10)
# plt.plot(c_max)
# plt.plot(c_min)
# plt.plot(cc[N_start:])
#
# plt.plot(np.dot(M_noise_10, N0_noise_10) - deaths)
